...
class AbstractBlack76Model(AbstractOptionModel):
    ...
    def __init__(self, S: float, K: float, r: float, sigma: float, T: float):
        ...
        print("check222: ", self.F, self.K, self.r, self.sigma, self.T, self.F, self.d1, self.d2, self.discount_factor)
        print("d1: ", (np.log(self.F / self.K) + self.sigma**2 / 2 * self.T) / (self.sigma * np.sqrt(self.T)))
        print("d2: ", (np.log(self.F / self.K) - self.sigma**2 / 2 * self.T) / (self.sigma * np.sqrt(self.T)))
